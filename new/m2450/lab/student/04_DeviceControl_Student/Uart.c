...
void Uart_Init(int baud)
{
    ...
    //rUBRDIV1= ((unsigned int)(660000000/16./115200+0.5)-1 )
}
...
void Uart_Send_Byte(int data)
{
    if(data=='\n')
    {
        while(!(rUTRSTAT1 & 0x2));  //UART channel 1 Tx/Rx status register
        // UTRSTAT1 0x50004010 R UART channel 1 Tx/Rx status register
        // 0 =The buffer register is not empty
        WrUTXH1('\r');  // UTXH1 0x50004020 W (by byte) UART channel 1 transmit buffer register
    }

    while(!(rUTRSTAT1 & 0x2));	
    // WrUTXH1(data);
    // (*(volatile unsigned char *)0x50004023)=(unsigned char)(data);
    (*(volatile unsigned char *)0x50004020)=(unsigned char)(data);
}
...
char Uart_GetChar(void)
{
    ...
    /* YOUR CODE HERE */  
    while(!(rUTRSTAT1 & (1<<0)));   // 1 = The buffer register has a received data
    // (*(volatile unsigned char *)0x50004024), rURXH1
    return rURXH1;
    // RdURXH1();
    // rURXH1;
}
